...
class PushMessageTests(unittest.TestCase):
    ...
    def test_build_open_not_chase_shadow_observations_marks_only_normal_rows_eligible(self) -> None:
        ...
        self.assertEqual(str(out.iloc[0]["heat_policy_state"]), "hot_trend")
        self.assertEqual(str(out.iloc[0]["heat_participation_bias"]), "小倉參與")
        self.assertTrue(bool(out.iloc[0]["shadow_eligible"]))
        self.assertEqual(str(out.iloc[0]["manual_trial_cap"]), "<= 1/4 test position")
        self.assertIn("收盤破前低或 1 ATR 出", str(out.iloc[0]["manual_trial_rule"]))
