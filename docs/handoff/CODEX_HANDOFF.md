...
- `python -m stock_watch doctor --skip-network` 現在除了 `local_doctor.md/json`，還會寫 `runs/theme_watchlist_daily/local_doctor_summary.txt`，給排程 / quick glance 用；`tools/scheduling/stock-watch-postclose.sh` 會用 `--fail-on warn` 當成每日健康 gate。
